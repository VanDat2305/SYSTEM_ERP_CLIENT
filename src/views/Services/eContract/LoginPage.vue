<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
    <div class="w-full max-w-md p-8 bg-white dark:bg-gray-800 rounded-2xl shadow">
      <h2 class="mb-6 text-center text-2xl font-bold text-gray-900 dark:text-white">Đăng nhập dịch vụ eContract</h2>
      <form @submit.prevent="login">
        ...
        <button type="submit" :disabled="loading"
          class="w-full py-2 rounded-lg bg-gradient-to-br from-orange-500 to-amber-600 text-white font-bold transition">
...
async function login() {
  ...
  try {
    ...
    localStorage.setItem('account_token_contract', token)
    localStorage.setItem('account_info',  JSON.stringify(res.data?.data.account || {}))
    
    
    router.push('/econtract/my-services')
  } catch (err) {
