<template>
  <div class="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 dark:from-gray-900 dark:via-gray-800 dark:to-orange-900/20 py-8 px-4">
    <div class="max-w-6xl mx-auto">
      ...
      <!-- Company Info -->
      <div v-if="getInfo().name || getInfo().tax_code" class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg mb-8 border border-orange-100 dark:border-orange-900/20">
        <div class="flex items-center mb-4">
          <div>
            <p class="text-sm text-gray-500 dark:text-gray-400">Chi tiết tài khoản</p>
          </div>
        </div>
        <div class="grid md:grid-cols-2 gap-4">
          <div v-if="getInfo().name" class="flex items-center text-sm text-gray-600 dark:text-gray-400">
            <svg class="w-4 h-4 mr-2 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
            </svg>
            <span class="font-medium">{{ getInfo().name }}</span>
          </div>
          <div v-if="getInfo().tax_code" class="flex items-center text-sm text-gray-600 dark:text-gray-400">
            <svg class="w-4 h-4 mr-2 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
            </svg>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-orange-100 dark:border-orange-900/20 hover:shadow-xl transition-all duration-300">
          <div class="flex items-center">
            <div class="p-3 bg-gradient-to-br from-orange-500 to-amber-600 rounded-full shadow-md">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-orange-100 dark:border-orange-900/20 hover:shadow-xl transition-all duration-300">
          <div class="flex items-center">
            <div class="p-3 bg-gradient-to-br from-yellow-500 to-orange-500 rounded-full shadow-md">
              <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1"></path>
              </svg>
            </div>
            <div class="ml-4">
              <p class="text-sm text-gray-600 dark:text-gray-400">Số lượng còn lại</p>
              <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ totalRemaining }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-else-if="filteredServices.length === 0" class="text-center py-16">
        <div class="mx-auto h-24 w-24 text-orange-300 mb-4">
      ...
      <div v-else class="space-y-8">
        <div v-for="service in filteredServices" :key="service.service_type">
          ...
          <div class="flex items-center mb-6">
            <div class="flex-shrink-0 w-14 h-14 bg-gradient-to-br from-orange-500 to-amber-600 rounded-full flex items-center justify-center shadow-lg">
              <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
              </svg>
            </div>
            <div class="ml-4">
              <h2 class="text-xl font-bold text-gray-900 dark:text-white">Dịch vụ eContract</h2>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div v-for="pkg in service.packages" :key="pkg.order_detail_id"
              class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-orange-100 dark:border-orange-900/20 hover:border-orange-300 dark:hover:border-orange-700">
                ...
                <div class="flex items-start justify-between">
                  ...
                  <div class="flex-shrink-0">
                    <span :class="pkg.is_active 
                      ...
                      {{ pkg.is_active ? 'Đã kích hoạt' : 'Chưa kích hoạt' }}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div class="px-6 pb-6">
                <div class="space-y-4">
                  <div v-for="feature in pkg.features.filter(f => f.feature_type === 'quantity')"
                    ...
                    class="border border-orange-200 dark:border-orange-800 rounded-xl p-4 bg-gradient-to-r from-orange-50 to-amber-50 dark:from-gray-700 dark:to-gray-600 hover:from-orange-100 hover:to-amber-100 dark:hover:from-gray-600 dark:hover:to-gray-500 transition-all duration-200">
                    
                    <div class="flex items-center justify-between mb-3">
                      <h4 class="font-semibold text-gray-900 dark:text-white">
                        {{ feature.feature_name }}
                      </h4>
                      <span class="text-sm text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-800 px-2 py-1 rounded-md">
                        {{ feature.unit || 'lượt' }}
                      </span>
                    </div>
                    ...
                    <div class="mb-3" v-if="feature.feature_key != 'duration'">
                      <div class="flex justify-between text-sm text-gray-600 dark:text-gray-400 mb-2">
                        <span>Đã sử dụng: <strong>{{ feature.total_quota - feature.remain }}</strong></span>
                        <span>Còn lại: <strong class="text-orange-600">{{ feature.remain }}</strong></span>
                      </div>
                      <div class="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-3 overflow-hidden">
                        <div class="bg-gradient-to-r from-orange-500 to-amber-500 h-3 rounded-full transition-all duration-500 shadow-sm" 
                    ...
                    <div class="flex items-center justify-between">
                      <div class="flex items-center space-x-2">
                        ...
                        <input
                          ...
                          class="w-20 px-3 py-2 text-sm border border-orange-300 dark:border-orange-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all duration-200"
                      ...
                      <button
                        ...
                        class="inline-flex items-center px-6 py-2 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 disabled:from-gray-400 disabled:to-gray-400 text-white text-sm font-medium rounded-lg shadow-md hover:shadow-lg transition-all duration-200 transform hover:scale-105 disabled:hover:scale-100">
                        ...
                        {{ loadingUse[`${pkg.order_detail_id}-${feature.feature_key}`] ? 'Đang xử lý...' : 'Ký'}}
                      </button>
                      <div v-else-if="!pkg.is_active" class="text-sm text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-md">
                        Dịch vụ đã hết hạn
                      </div>
                      <div v-else class="text-sm text-orange-600 dark:text-orange-400 bg-orange-100 dark:bg-orange-900 px-3 py-1 rounded-md">
      ...
      <!-- Messages -->
      <div v-if="error" class="mt-8 p-4 bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-800 rounded-2xl">
        <div class="flex items-center">
      ...
      <div v-if="successMsg" class="mt-8 p-4 bg-green-50 dark:bg-green-900 border border-green-200 dark:border-green-800 rounded-2xl">
        <div class="flex items-center">
...
function getToken() {
  return localStorage.getItem('account_token_contract')
}
function getInfo() {
...
const filteredServices = computed(() =>
  services.value
    .filter(s => s.service_type === 'SER_EC')
    .map(s => ({
      ...s,
...
async function useFeature(order_detail_id, feature_key) {
  ...
  try {
    ...
    successMsg.value = `Ký thành công.`
    await fetchServices()
