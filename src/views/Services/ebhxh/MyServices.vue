<template>
  <div class="min-h-screen bg-gradient-to-br from-red-50 to-rose-100 dark:from-gray-900 dark:to-gray-800 py-8 px-4">
    <div class="max-w-5xl mx-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg">
          <div class="flex items-center">
            <div class="p-3 bg-rose-50 dark:bg-rose-900 rounded-full">
              <svg class="w-6 h-6 text-rose-600 dark:text-rose-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="loading" class="text-center py-16">
        <div class="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-rose-600"></div>
      ...
      <div v-else-if="filteredServices.length === 0" class="text-center py-16">
        <div class="mx-auto h-24 w-24 text-rose-400">
      ...
      <div v-else class="space-y-8">
        <div v-for="service in filteredServices" :key="service.service_type">
          ...
          <div class="flex items-center mb-6">
            <div class="flex-shrink-0 w-12 h-12 bg-rose-100 dark:bg-rose-900 rounded-full flex items-center justify-center">
              <svg class="w-6 h-6 text-rose-600 dark:text-rose-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
              </svg>
            </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div v-for="pkg in service.packages" :key="pkg.order_detail_id"
              ...
              <div class="px-6 pb-6">
                <div class="space-y-4">
                  <div v-for="feature in pkg.features.filter(f => f.feature_type === 'quantity')" :key="feature.feature_key"
                    class="border dark:border-gray-700 rounded-lg p-4 bg-red-50 dark:bg-gray-700">
                    ...
                    <div class="mb-3" v-if="feature.feature_key != 'duration'">
                      ...
                      <div class="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-2">
                        <div class="bg-rose-600 h-2 rounded-full transition-all duration-300"
                    ...
                    <div class="flex items-center justify-between">
                      <div class="flex items-center space-x-2">
                        ...
                        <input
                          ...
                          class="w-20 px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-rose-500 focus:border-rose-500"
                      ...
                      <button
                        ...
                        class="inline-flex items-center px-4 py-2 bg-rose-600 hover:bg-rose-700 disabled:bg-gray-400 text-white text-sm font-medium rounded-lg transition-colors duration-200">
                        ...
                        {{ loadingUse[`${pkg.order_detail_id}-${feature.feature_key}`] ? 'Đang xử lý...' : 'Nộp tờ khai' }}
                      </button>
      ...
      <div v-if="successMsg" class="mt-8 p-4 bg-rose-50 dark:bg-rose-900 border border-rose-200 dark:border-rose-800 rounded-lg">
        <div class="flex items-center">
          <svg class="w-5 h-5 text-rose-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
          </svg>
          <span class="text-rose-800 dark:text-rose-200">{{ successMsg }}</span>
        </div>
      </div>
    </div>
  </div>
...
function getToken() {
  return localStorage.getItem('account_token_ebhxh')
}
function getInfo() {
...
const filteredServices = computed(() =>
  services.value
    .filter(s => s.service_type === 'SER_BHXH')
    .map(s => ({
      ...s,
...
async function useFeature(order_detail_id, feature_key) {
  ...
  try {
    ...
    successMsg.value = `Nộp tờ khai thành công.`
    await fetchServices()
