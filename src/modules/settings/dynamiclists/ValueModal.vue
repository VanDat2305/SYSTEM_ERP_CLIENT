...
const validateField = (field) => {
  if (field === 'code') {
    if (!formData.value.code.trim()) {
      errors.value.code = t('validation.required', { field: t('dynamic_lists.catalog-values.fields.code') });
    } else if (formData.value.code.length > 50) {
      errors.value.code = t('validation.max_length', { field: t('dynamic_lists.catalog-values.fields.code'), count: 50 });
    } else {
  ...
  if (field === 'name') {
    if (!formData.value.name.trim()) {
      errors.value.name = t('validation.required', { field: t('dynamic_lists.catalog-values.fields.name') });
    } else if (formData.value.name.length > 255) {
      errors.value.name = t('validation.max_length', { field: t('dynamic_lists.catalog-values.fields.name'), count: 255 });
    } else {
